...
\title{Discovering Adverse Drug Reactions for New FDA Approved Drugs Using Google Search Results}
%
...
% 1st. author
\alignauthor
Vignesh Raja, Sheng Wang, ChengXiang Zhai\\
       ...
       \email{\{vraja2,swang141,czhai\}@illinois.edu}
       %\email{vraja2@illinois.edu,swang141@illinois.edu,czhai@illinois.edu}
}
...
\maketitle
\begin{abstract}
When the Food and Drug Administration (FDA) approves new drugs, it often takes years of public use for exhaustive adverse drug reaction (ADR) lists to be constructed. Oftentimes, the ADRs reported to the FDA are of poor quality. In recent years, patients have been posting their experiences with drugs on health forums, blogs, etc. In this paper, we construct a system to retrieve ADRs from Google search results. To take advantage of online information, we need to be able to discriminate between text that contains information about ADRs and text that does not. First, we create a classifier trained on sentence-level data from Ask a Patient, a website allowing patients to post about their experiences with specific drugs. We use unigrams, bigrams, trigrams, dependency parse information, and side-effects crawled from Side Effect Resource (SIDER) as features for our classifier. We perform evaluation using 12-fold cross validation on the Ask a Patient data which yields convincing results. Our overall accuracy is $84.10\%$, our F1 score on positive samples is $83.98$, and our F1 score on negative samples is $84.21$. In the next step, we use our classifier on the Google search results for a specific drug. We retrieve all the sentence classified as containing ADRs (positive samples), create bigrams, and sort based on bigram frequency. We similarly obtain bigram frequencies from the set of all sentences (both positive and negative samples) in the Google search results. We found that when considering the top-50 most frequent bigrams, recall of ADRs was higher for the set of positively classified sentences which shows that our classifier extends well to unseen Google search data.
%Next, we retrieve the top $10$ search results for the query ``<drug name> side effects'', where ``<drug name>'' is the drug we are interested in finding ADRs for. We extract the text from the obtained webpages, and use our classifier to find sentences containing ADRs. Using exclusively positively classified sentences, 
\end{abstract}
...
\captionof{table}{Unigrams + Bigrams Results}
\vspace{-3mm}
\tabcolsep=0.11cm
...
\captionof{table}{Unigrams + Bigrams + Trigrams Results}
\vspace{-3mm}
\tabcolsep=0.11cm
...
\captionof{table}{Unigrams + Bigrams + Dependency Parse Results}
\vspace{-3mm}
\tabcolsep=0.11cm
\resizebox{\linewidth}{1cm}{
\begin{tabular}{| l | l | l | l | l | l | l | l | l | l |}
...
\captionof{table}{Unigrams + Bigrams + Dependency Parse + SIDER Results}
\vspace{-3mm}
\tabcolsep=0.11cm
\begin{tabular}{| l | l | l | l | l | l | l | l |}
\hline
...
\captionof{table}{ADR Unigram Results}
\vspace{-3mm}
\tabcolsep=0.11cm
\begin{tabular}{| l | l |}
\hline
...
\captionof{table}{ADR Bigram Results}
\vspace{-3mm}
\tabcolsep=0.11cm
